...
# ini_set <- c(trans_par_set(par_set), trans_par_set_sub(par_set_sub[-c(same_level_sub, whichZero)]))
# 
# # value of objective function at the initial value
# x1 <- Sys.time()
# loglikelihood(ini_set)
# Sys.time() - x1 # time taken to calculate likelihood once
# 
# # Optimization
# 
# #
# # result_optim <- optim(ini_set,
# #       loglikelihood,
# #       method = c("CG"),
# #       lower = -3, upper = 5,
# #       control = list(reltol = 1e1, maxit = 2, trace = 1),
# #       hessian = FALSE)
# s1 <- Sys.time()
# result_solnp[[i]] <- solnp(pars = ini_set, #log odds transformation
#                           fun = loglikelihood,
#                           control = list(
#                             # inner.iter = 2,
#                             # outer.iter = 5,
...
# result_solnp$values
# tym_to_run_optimization <- s2 - s1
# # Estimated Demand Parameters
# par_set_est <- result_solnp$pars[seq(1, length(par_set), 1)] %>% reparam_old()
# # Estimated Substitution Parameters
# par_set_sub_est <- result_solnp$pars[seq(length(par_set) + 1, length(result_solnp$pars), 1)] %>% reparam_sub() %>% 
#   same_level_sub_addition(., num = 1, toReplace = same_level_sub) %>% same_level_sub_addition(., num = 0, toReplace = whichZero)
# 
# options(error=recover)
# options(warning.expression=quote(recover()))
# options(warn = -1)
